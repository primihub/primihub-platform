...
public class OrganConfiguration {
    ...
    private void readNacosBaseConfig() {
        try {
            ...
            String organInfoContent = configService.getConfig(SysConstant.BASE_JSON, group, 3000);
            log.info(" nacos base.json data:{}", organInfoContent);
            if (StringUtils.isNotBlank(organInfoContent)) {
                baseConfiguration = JSON.parseObject(organInfoContent, BaseConfiguration.class);
            }
            configService.addListener(SysConstant.BASE_JSON, group, new Listener() {
                @Override
                ...
                public void receiveConfigInfo(String config) {
                    log.info(" nacos receive ConfigInfo base.json data:{}", config);
                    if (StringUtils.isNotBlank(config)) {
                        baseConfiguration = JSON.parseObject(config, BaseConfiguration.class);
                    } else {
            ...
            log.info("nacos base.json Exception:{}", e.getMessage());
        }
    }
