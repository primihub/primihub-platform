 ...
 */
package com.primihub.biz.util.seatunnel;
...
public class SeaTunnelEngineProxy {
    ...
    private SeaTunnelEngineProxy() {
        String SEATUNNEL_HOME = System.getProperty("SEATUNNEL_HOME");
        log.info("SEATUNNEL_HOME:{}",SEATUNNEL_HOME);
        String HAZELCAST_CLIENT_CONFIG = System.getProperty("hazelcast.client.config");
        log.info("hazelcast.client.config:{}",HAZELCAST_CLIENT_CONFIG);
        if (!"".equals(SEATUNNEL_HOME)){
    ...
    public void restoreJobContent(
            @NonNull String content, @NonNull Long jobInstanceId, @NonNull Long jobEngineId) {
        String filePath = writeJobConfigIntoConfFile(content,jobEngineId);
        log.info(filePath);
        try {
            SeaTunnelClient seaTunnelClient = new SeaTunnelClient(clientConfig);
            JobConfig jobConfig = new JobConfig();
    ...
    public String writeJobConfigIntoConfFile(String jobContent, Long jobDefineId) {
        String projectRoot = System.getProperty("user.dir");
        String filePath = projectRoot + File.separator +"profile"+File.separator + jobDefineId + ".json";
        try {
            ...
            bufferedWriter.write(jobContent);
