...
//        dataAsyncService.deleteModel(vo);
    ...
    public BaseResultEntity runTaskModel(Long modelId, Long userId) {
        ...
        if (dataModel.getIsDraft().equals(ModelStateEnum.DRAFT.getStateType())) {
            log.error("[模型任务][运行失败] 模型未保存,请保存后再次尝试");
        ...
        if (StringUtils.isBlank(dataModel.getComponentJson())) {
            log.error("[模型任务][运行失败] 未查询到模型组件信息");
            return BaseResultEntity.failure(BaseResultEnum.DATA_RUN_TASK_FAIL, "未查询到模型组件信息");
        }
        // 检验是否有任务正在进行中
        Map<Long, Map<String, Object>> taskMap = dataModelRepository.queryModelLatestTask(new HashSet<Long>() {{
            add(modelId);
        }});
        if (taskMap != null && !taskMap.isEmpty()) {
            Map<String, Object> dataMap = taskMap.get(modelId);
            if (dataMap.get("taskState") != null && Integer.parseInt(dataMap.get("taskState").toString()) == TaskStateEnum.IN_OPERATION.getStateType()) {
                log.error("[模型任务][运行失败] 任务正在运行中");
                return BaseResultEntity.failure(BaseResultEnum.DATA_RUN_TASK_FAIL, "任务正在运行中");
            }
        }
        // 从模型任务中组装 taskReq 模型任务context
            ...
            if (CollectionUtils.isNotEmpty(modelProjectResourceVos)){
                ...
                if (CollectionUtils.isNotEmpty(organIdSet)){
                    ...
                    if (CollectionUtils.isEmpty(sysOrgans)){
                        ...
                        for (String organId : organIdSet){
                            if (!collect.contains(organId)){
                                log.error("[模型任务][运行失败] 模型参与节点不可用");
                                return BaseResultEntity.failure(BaseResultEnum.DATA_RUN_TASK_FAIL, "模型参与节点不可用");
                            }
                        }
