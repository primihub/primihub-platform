...
import com.primihub.biz.entity.data.dataenum.ModelProjectPartyRoleEnum;
...
public class DataSetComponentTaskServiceImpl extends BaseComponentServiceImpl implements ComponentTaskService {
    ...
    public BaseResultEntity runTask(DataComponentReq req, ComponentTaskReq taskReq) {
        // 选择数据集组件不需要寻找最新的资源
        Map<String, String> resourceMap = taskReq.getFusionResourceList()
                .stream().collect(Collectors.toMap(d -> d.get("resourceId").toString(), d -> d.get("resourceColumnNameList").toString()));
        for (int i = 0; i < taskReq.getResourceList().size(); i++) {
            ModelProjectResourceVo modelProjectResourceVo = taskReq.getResourceList().get(i);
            if (Objects.equals(modelProjectResourceVo.getParticipationIdentity(), ModelProjectPartyRoleEnum.HOST.getCode())) {
