...
export default {
  ...
  methods: {
    ...
    /** delete the specified query  */
    handelDeleteQueryParam(index) {
...
<template>
  <div>
    ...
    <template v-slot:container>
      <el-form ref="form" :model="form" :rules="rules" label-width="130px" class="demo-form">
        <ViewPart title="基本信息">
          <el-form-item label="任务流中文名称" prop="taskName">
            <el-input v-model="form.taskName" maxlength="50" show-word-limit placeholder="请输入任务流中文名称" />
          </el-form-item>
          <el-form-item label="任务流英文名称" prop="taskName">
            <el-input v-model="form.taskName" maxlength="50" show-word-limit placeholder="请输入任务流英文名称" />
          </el-form-item>
          <el-form-item label="任务流描述" prop="taskName">
            <el-input v-model="form.projectDesc" type="textarea" maxlength="200" show-word-limit placeholder="请输入任务流描述" />
          </el-form-item>
        </ViewPart>
        <ViewPart title="任务配置">
          ...
          <div class="param-container" v-for="(item, index) in params" :key="index">
            <el-form-item label="查询主键" prop="pirParam" label-width="110px">
              <el-select v-model="form.region" placeholder="请选择活动区域">
                <el-option label="区域一" value="shanghai"></el-option>
                <el-option label="区域二" value="beijing"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="关键词" prop="pirParam" label-width="110px">
              <tags-input :selected-data="tagValueList" @change="handleKeywordChange" />
              <p class="pirParam-tip">基于关键词的精准查询，输入关键词后Enter即可。</p>
            </el-form-item>
            <div class="param-operate-btn-wrapper">
              <i v-if="params.length > 1" class="el-icon-remove-outline param-operate-btn" @click="handelDeleteQueryParam(index)" />
              <i v-if="index === params.length - 1" class="el-icon-circle-plus-outline param-operate-btn" @click="handelAddQueryParam" />
            </div>
          </div>
        </ViewPart>
      </el-form>
    ...
    <template v-slot:footer>
      <el-button v-if="hasSearchPermission" type="primary" @click="next">查询<i class="el-icon-search el-icon--right" /></el-button>
    </template>
  </ViewForm>
